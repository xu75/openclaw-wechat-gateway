...
import { updateTaskAndAppendEventAtomic } from './task-event.atomic.js';
...
test('task update and event append can commit in the same transaction', async () => {
  const { dbPath, cleanup } = createTempDbPath();
  try {
    runMigrations(dbPath);
    const db = openSqlite(dbPath);
    try {
      ...
      updateTaskAndAppendEventAtomic({
        db,
        tasks,
        events,
        taskId: task.task_id,
